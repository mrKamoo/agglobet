# Ajouter le lien Synchronisation dans la navigation admin
...
Ajouter le lien "Synchronisation" dans la barre de navigation après "Résultats" (ligne 42).
...
```blade
                                ...
                                <a href="{{ route('admin.sync.index') }}" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('admin.sync.*') ? 'border-indigo-400 text-white' : 'border-transparent text-gray-100 hover:text-white hover:border-gray-400' }} text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out">
                                    Synchronisation
                                </a>
                            </div>
```
...
Remplacez tout le contenu du fichier `resources/views/components/admin-layout.blade.php` par :
...
Une fois cette modification effectuée, vous verrez un nouveau lien **"Synchronisation"** dans la barre de navigation admin, à côté de "Résultats".

Le lien sera automatiquement mis en surbrillance (bordure bleue) lorsque vous êtes sur la page de synchronisation.
