          ...
          <div ref="gifPickerContainer" class="relative flex items-center">
            ...
            >
              ...
              <div class="h-48 overflow-y-auto min-h-[12rem] relative">
                ...
                <div v-else-if="gifError" class="flex flex-col items-center justify-center h-full text-center text-xs text-red-500 p-3">
                  <svg class="w-8 h-8 mb-1.5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                  </svg>
                  <span class="font-semibold" v-if="gifError === 'MISSING_KEY'">Clé Giphy non configurée</span>
                  <span class="font-semibold" v-else>Erreur API Giphy</span>
                  <span class="text-[10px] text-gray-400 mt-1">Veuillez renseigner <strong>GIPHY_API_KEY</strong> dans le fichier <strong>.env</strong> de l'application.</span>
                </div>
  ...
  methods: {
    ...
    async fetchGifs() {
      ...
      try {
        ...
        if (error.response && error.response.data && error.response.data.code) {
          this.gifError = error.response.data.code;
        } else {
          this.gifError = 'API_ERROR';
        }
        this.gifs = [];
      } finally {
    ...
    async confirmDeleteMessage(message) {
      ...
      try {
        ...
        alert("Impossible de supprimer le message. Veuillez réessayer.");
      }
    },
